		...
		{for i 1 52}
			<tr>
				...
				<ul>
					...
					<li><a style="color:{$event->color}" href="{$WWWROOT}interaction/schedule/{if $admin}editevent.php?id={else}view.php?event={/if}{$event->id}&view={$view}" title="{$event->startdate|format_date:'strftimetime'}&#13;{$event->scheduletitle}&#13;{str tag=where section=interaction.schedule}{$event->location}">{$event->title}</a></li>	
					{/foreach}
				</ul>
				</div>
				</td>
