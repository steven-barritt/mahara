    {if $foruminfo}
        ...
        {foreach from=$foruminfo item=postinfo}
            <tr class="{cycle values='r0,r1'}">
                ...
                <td class="valign right s"><a href="{profile_url($postinfo->author)}"><img src="{profile_icon_url user=$postinfo->author maxheight=20 maxwidth=20}"  style="max-width:20px;max-height:20px;" alt="{str tag=profileimagetext arg1=$postinfo->author|display_default_name}"> {$postinfo->author|display_name}</a>
                </td>
            </tr>
        {/foreach}
        </table>
